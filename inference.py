...
def main(args):
    ...
    path_of_lm_croper = os.path.join(
        current_root_path, args.checkpoint_dir, "shape_predictor_68_face_landmarks.dat"
    )
    path_of_net_recon_model = os.path.join(
        current_root_path, args.checkpoint_dir, "epoch_20.pth"
    )
    dir_of_BFM_fitting = os.path.join(
        current_root_path, args.checkpoint_dir, "BFM_Fitting"
    )
    wav2lip_checkpoint = os.path.join(
        current_root_path, args.checkpoint_dir, "wav2lip.pth"
    )

    audio2pose_checkpoint = os.path.join(
        current_root_path, args.checkpoint_dir, "auido2pose_00140-model.pth"
    )
    ...
    audio2exp_checkpoint = os.path.join(
        current_root_path, args.checkpoint_dir, "auido2exp_00300-model.pth"
    )
    ...
    free_view_checkpoint = os.path.join(
        current_root_path, args.checkpoint_dir, "facevid2vid_00189-model.pth.tar"
    )

    mapping_checkpoint = os.path.join(
        current_root_path, args.checkpoint_dir, "mapping_00109-model.pth.tar"
    )
    ...
    enhancer_model = FaceEnhancement(
    ...
    with Timer("3DMM Extraction for source image"):
        first_coeff_path, crop_pic_path, crop_info = preprocess_model.generate(
            pic_path, first_frame_dir
        )
    ...
    with Timer("coeff2video"):
        data = get_facerender_data(
            coeff_path, crop_pic_path, first_coeff_path, audio_path, batch_size, device
        )
    with Timer("animate_from_coeff"):
    ...
    if args.use_DAIN:
        with Timer("DAIN"):
            ...
            predictor_dian = dain_predictor.DAINPredictor(
                args.dian_output,
        ...
        with Timer("ffmpeg"):
            ...
            command = (
                r'ffmpeg -hide_banner -loglevel error  -y -i "%s" -i "%s" -vcodec copy "%s"'
                % (
                    temp_video_path,
                    new_audio_path,
                    save_path,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source_video",
        default="./examples/source_image/input.mp4",
    ...
    parser.add_argument(
        ...
        type=str,
        default="lip",
        help="enhaner region:[none,lip,face] \
                                                                      none:do not enhance; \
    ...
    parser.add_argument(
        "--DAIN_weight",
        type=str,
        default="./checkpoints/DAIN_weight",
        help="Path to model weight",
    )
    parser.add_argument(
        "--dian_output", type=str, default="dian_output", help="output dir"
    )
    parser.add_argument(
